...
static struct mag_hw *get_cust_mag_hw(void)
{
	return &mag_cust;
}
////....
...
static int mxg2320_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
...
{
////...JamesNote: This need to double check with MTK Engineer.
}
#if 0
{
...
static long mxg2320_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ( argp == NULL ) {
        ...
        case MSENSOR_IOCTL_READ_SENSORDATA:
                ...
                mxg2320_operation_mode(MXG2320_MOD_SNG_MEASURE);/*JamesNeed_ForAndroid_M.*/
        ...
        case MSENSOR_IOCTL_READ_SENSORDATA_MXDAEMON:
                MSE_ERR("MSENSOR_IOCTL_READ_SENSORDATA_MXDAEMON\n");
                ////mxg2320_operation_mode(MXG2320_MOD_SNG_MEASURE);//JamesNoNeed_ForAndroid_M_Daemon.
                mxg2320_get_raw_data(MXG2320_MOD_SNG_MEASURE);
                read_lock(&data->mdata.datalock);
                raw_data[0] = data->mdata.umag.ux;
                raw_data[1] = data->mdata.umag.uy;
...
static int mxg2320_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strcpy(info->type, MXG2320_DEV_NAME);
	return 0;
}
...
/*lenovo-sw caoyi1 add begin*/
	...
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		MSE_ERR("Cannot find mag pinctrl!\n");
	}
	pins_default = pinctrl_lookup_state(pinctrl, "pin_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		MSE_ERR("Cannot find mag pinctrl default!\n");
	}

	pins_cfg = pinctrl_lookup_state(pinctrl, "pin_cfg");
	if (IS_ERR(pins_cfg)) {
		ret = PTR_ERR(pins_cfg);
		MSE_ERR("Cannot find mag pinctrl pin_cfg!\n");
...
static int mxg2320_init_device(struct mxg2320_i2c_data *data)
{
	...
	#ifdef MXG2320_MTK_NEW_SENSOR_ARCH
	    data->hw = hw;
	    MSE_ERR("mxg2320 data->hw->direction: %d\n", data->hw->direction);
	#else
...
////aaaaaaaaaaaaaaaafff
	...
	if (!hw)
		MSE_INF("mxg2320 get dts info fail\n");
...
{
#ifndef MXG2320_MTK_NEW_SENSOR_ARCH
    platform_driver_unregister(&mxg2320_sensor_driver);
#endif
}
