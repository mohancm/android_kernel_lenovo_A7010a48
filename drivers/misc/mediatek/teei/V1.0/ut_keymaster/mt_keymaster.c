...
#define KEYMASTER_SIZE	0x80000
#define DEV_NAME "mt_keymaster"
...
#define MT_KEYMASTER_BUF_SIZE = 512*1024;
...
void keymaster_init(void)
{
	...
	if (IS_ERR(driver_class)) {
		result = -ENOMEM;
		printk("mt_keymaster class_create failed %d.\n", result);
	...
	if (!class_dev) {
		result = -ENOMEM;
		printk("mt_keymaster class_device_create failed %d.\n", result);
		goto class_destroy;
	}
	keymaster_devp = NULL;
	...
	printk("[%s][%d]create the mt_keymaster device node successfully!\n", __func__,
			__LINE__);
	goto return_fn;
