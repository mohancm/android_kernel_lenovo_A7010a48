...
struct pinctrl_state *cam0_mclk0 = NULL;
struct pinctrl_state *cam0_mclk1 = NULL;
struct pinctrl_state *cam1_mclk0 = NULL;
struct pinctrl_state *cam1_mclk1 = NULL;
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	#if 1
	cam0_mclk0 = pinctrl_lookup_state(camctrl, "cam0_mclk0");
	if (IS_ERR(cam0_mclk0)) {
		ret = PTR_ERR(cam0_mclk0);
		pr_debug("%s : pinctrl err, cam0_mclk0\n", __func__);
	}
	cam0_mclk1 = pinctrl_lookup_state(camctrl, "cam0_mclk1");
	if (IS_ERR(cam0_mclk1)) {
		ret = PTR_ERR(cam0_mclk1);
		pr_debug("%s : pinctrl err, cam0_mclk1\n", __func__);
	}
	cam1_mclk0 = pinctrl_lookup_state(camctrl, "cam1_mclk0");
	if (IS_ERR(cam1_mclk0)) {
		ret = PTR_ERR(cam1_mclk0);
		pr_debug("%s : pinctrl err, cam1_mclk0\n", __func__);
	}
	cam1_mclk1 = pinctrl_lookup_state(camctrl, "cam1_mclk1");
	if (IS_ERR(cam1_mclk1)) {
		ret = PTR_ERR(cam1_mclk1);
		pr_debug("%s : pinctrl err, cam1_mclk1\n", __func__);
	}
	#endif
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case CAMMCLK:
		...
		if (PinIdx == 0) {
			if (Val == 0)
				pinctrl_select_state(camctrl, cam0_mclk0);
			else
				pinctrl_select_state(camctrl, cam0_mclk1);
		} else {
			if (Val == 0)
				pinctrl_select_state(camctrl, cam1_mclk0);
			else
				pinctrl_select_state(camctrl, cam1_mclk1);
		}
		break;
	/*lenovo.sw wuyt3 add end*/
	default:
...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if(1){
			mtkcam_gpio_set(pinSetIdx, CAMMCLK, IDX_LDO_ON);
			mdelay(3);
			...
			/* Main VCAMD max 1.5V */
				if(pinSetIdx == 1){
					...
					mtkcam_gpio_set(pinSetIdx, CAMLDO, IDX_LDO_ON);
		...
		if (pinSetIdx == 0)
			...
			ISP_MCLK2_EN(0);
		{
			mtkcam_gpio_set(pinSetIdx, CAMMCLK, IDX_LDO_OFF);
			...
			if(pinSetIdx==0){
				...
				mtkcam_gpio_set(pinSetIdx, CAMLDO, IDX_LDO_OFF);
			}
		}
