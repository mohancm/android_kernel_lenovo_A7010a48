...
//#define DEBUG_GPIO_SPI
#define SUPPORT_GPIO_SPI
#define USE_MT_GPIO
#ifdef USE_MT_GPIO
...
static inline void setsck(const struct spi_device *spi, u32 is_on)
{
	printk("setsck=%d\n",is_on);

#ifdef USE_MT_GPIO
       mt_set_gpio_out_base(SPI_SCK_GPIO, is_on);
#else
...
static inline void setmosi(const struct spi_device *spi, u32 is_on)
{
	printk("setmosi=%d\n",is_on);
...
static inline u32 getmiso(const struct spi_device *spi)
{
	//printk("getmiso\n");
...
{
#ifdef DEBUG_GPIO_SPI
		printk("spi_gpio_txrx_word_mode0 word=%x,bits=%d,nsecs=%x\n",word,bits,nsecs);
#endif
...
static int spi_gpio_setup(struct spi_device *spi)
{
	...
	if (np) {
		 ...
		 */
#ifdef USE_MT_GPIO
		pcs=(unsigned int*)(uintptr_t)spi->controller_data;
		cs=*pcs;
#else
...
static int spi_gpio_alloc(unsigned pin, const char *label, bool is_in)
{
	...
	//printk("spi_gpio_allocstart\n");

	if(Spmode!=NULL)
	{
		if(SPI_MISO_GPIO==pin)
			mode=Spmode->miso_mode;
		else if(SPI_MOSI_GPIO==pin)
			mode=Spmode->mosi_mode;
		else if(SPI_SCK_GPIO==pin)
			mode=Spmode->sck_mode;
        #ifdef DEBUG_GPIO_SPI
		...
		mt_set_gpio_mode_base(pin, mode);
#endif
if (value == 0) {
	#ifdef USE_MT_GPIO
...
printk("spi_gpio_request start\n");
	...
	printk("spi_gpio_request start1\n");
	...
	if (value)
		goto free_miso;
	printk("spi_gpio_request start2\n");
...
unsigned int gpio_sck,gpio_miso,gpio_mosi,num_chipselects;
	...
	ret =1;
  #ifdef DEBUG_GPIO_SPI	
	 printk("spi_gpio_probe_dt start\n");
   #endif	
#endif
	if (!of_id)
		return 0;
...
if (np)
{
   	...
   	if (of_property_read_u32_index(np, "num-css", 0, &num_chipselects))
   {
	...
	printk("num_cs=%d,mosi=%d,miso=%d,sck=%d\n",num_chipselects,gpio_mosi,gpio_miso,gpio_sck);
...
printk("spi_gpio_probe end5status=%d,maflag=%d\n",status,master_flags);
	if (status < 0)
		return status;

	master = spi_alloc_master(&pdev->dev, sizeof(*spi_gpio) +
					(sizeof(int) * SPI_N_CHIPSEL));
		printk("spi_gpio_probe end3\n");
	if (!master) {
			printk("spi_gpio_probe end4\n");
		status = -ENOMEM;
		goto gpio_free;
	}
	...
	printk("spi_gpio_probe ms_busn=%d,use_of=%d\n",master->bus_num,use_of);
#ifdef CONFIG_OF
...
#ifdef DEBUG_GPIO_SPI	
		printk("spi_gpio_probe end status=%d\n",status);
#endif
	if (status < 0) {
gpio_free:
		if (SPI_MISO_GPIO != SPI_GPIO_NO_MISO)
